...
class model(object):
    def __init__(vocab_size=6,\
                embedding_size=100,\
                maxlength=200,\
                initial_training_rate=0.001,\
                batch_size=1,\
                n_hidden = 512):#n_hidden无效了...现在是第一层512第二层256
# Target log path
        self.logs_path = 'log/rnn_words'
        self.writer = tf.summary.FileWriter(self.logs_path)
#learning_rate
        self.global_step = tf.Variable(0, trainable=False)
        self.learning_rate = tf.train.exponential_decay(initial_learning_rate, global_step=self.global_step, decay_steps=500,decay_rate=0.8)
# tf Graph input
