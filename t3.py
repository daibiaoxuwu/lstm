...
# tf Graph input
x = tf.placeholder("float", [None, n_input, embedding_size])
...
def RNN(inputs, weights, biases, pads,answers):
    ...
    # generate prediction
    outputs, states = tf.nn.dynamic_rnn(rnn_cell, x, pads, dtype=tf.float32)
...
# Model evaluation
    ...
    session.run([optimizer, accuracy, cost, pred], feed_dict={x: inputs, y: answers, pads:pads})#???
